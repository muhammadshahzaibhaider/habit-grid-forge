  ...
  return (
    <div className="border-2 border-border bg-card p-4 space-y-4">
      ...
      <div className="flex gap-2 justify-center">
        <Button size="sm" onClick={startStop} className="gap-1" variant={isRunning ? "secondary" : "default"}>
          {isRunning ? <Pause size={14} /> : <Play size={14} />}
          {isRunning ? "Pause" : "Start"}
        </Button>
        <Button size="sm" variant="outline" onClick={addLap} disabled={!isRunning} className="gap-1">
          <Flag size={14} />
          Lap
        </Button>
